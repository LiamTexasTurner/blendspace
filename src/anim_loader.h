...
ModelAnimation *LoadModelAnimations_ls(const char *fileName, int *animCount)
{
      // glTF file loading
      int dataSize = 0;
            ...
            if (data->skins_count > 0)
            {
                  ...
                  *animCount = (int)data->animations_count;
                  ...
                  for (unsigned int i = 0; i < data->animations_count; i++)
                  {
                        ...
                        Channels *boneChannels = (Channels*)RL_CALLOC(animations[i].boneCount, sizeof(struct Channels));
